...
const PlaceOrder = () => {
  ...
  const [method, setMethod] = useState('contraentrega');
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      switch (method) {
        ...
        // Llamada de API para Contraentrega
        case 'contraentrega':
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t' >
      ...
      <div className=' flex flex-col gap-4 w-full sm:max-w-[480px]' >
        ...
        <div className=' flex gap-3' >
          ...
          <input required onChange={onChangeHandler} name='lastName' value={formData.lastName} className=' border border-gray-300 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Apellidos' />

        </div>
        <input required onChange={onChangeHandler} name='email' value={formData.email}  className=' border border-gray-300 rounded py-1.5 px-3.5 w-full' type="email" placeholder='Correo Electrónico' />
        ...
        <div className=' flex gap-3' >
          ...
          <input required onChange={onChangeHandler} name='state' value={formData.state}  className=' border border-gray-300 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Provincia' />
        </div>
        <div className=' flex gap-3' >
      ...
      <div className=' mt-8 ' >
        ...
        <div className='mt-12' >
          ...
          <div className=' flex gap-3 flex-col lg:flex-row ' >
            ...
            <div onClick={() => setMethod('contraentrega')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer' >
              <p className={` min-w-3.5 h-3.5 border rounded-full ${method === 'contraentrega' ? 'bg-green-400' : ''} `} ></p>
              <p className=' text-gray-500 text-sm font-medium mx-4' >PAGO CONTRAENTREGA</p>
            </div>
          </div>

          <div className=' w-full text-end mt-8' >
            <button type='submit' className=' bg-black text-white px-16 py-3 text-sm' >HACER PEDIDO</button>
